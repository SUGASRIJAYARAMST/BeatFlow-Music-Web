...
if (missingVars.length > 0) {
  console.warn("⚠️ Missing ENV variables:", missingVars.join(", "));
  console.warn("⚠️ Server will start but may fail later...");
}
...
try {
  ...
  console.warn("⚠️ Clerk middleware failed:", err.message);
}

// ✅ FILE UPLOAD (SAFE TEMP DIR)
app.use((req, res, next) => {
  try {
    fileUpload({
      useTempFiles: true,
    ...
    console.warn("⚠️ File upload init failed:", err.message);
    next();
  }
});
...
const startServer = async () => {
  try {
    ...
    // ❗ DO NOT EXIT → Render will show Bad Gateway
  }
